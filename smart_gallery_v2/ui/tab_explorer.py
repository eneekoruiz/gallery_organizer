...
def render_semantic_explorer(db: DatabaseManager) -> None:
    ...
    if df.empty:
        st.info("No hay suficientes datos procesados por CLIP para generar la galaxia 3D. Ve a Triaje e indexa algunas imágenes.")
        return
    ...
    fig = px.scatter_3d(
        ...
        hover_data={"x": False, "y": False, "z": False, "date": True, "primary_tag": True, "triage_tier": False},
        color_discrete_map={
    ...
    st.caption("🔍 Usa el ratón para rotar (clic izquierdo), hacer zoom (rueda) y explorar tus constelaciones.")
